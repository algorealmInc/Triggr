...
export function ProjectSettingsPage({ project }: ProjectSettingsPageProps) {
  ...
  const [apiKey] = useState(`triggr_${project.id}_${Math.random().toString(36).substring(2, 15)}`);
  const [secretKey] = useState(`sk_${project.id}_${Math.random().toString(36).substring(2, 25)}`);
    ...
    <div className="p-6 space-y-8">
      ...
      <Card className="hover:shadow-elevated transition-all duration-300">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <Input value={project.contractNodeAddress} readOnly className="bg-muted flex-1 font-mono text-sm" />
              <Button
                variant="outline"
                size="icon"
                onClick={() => copyToClipboard(project.contractNodeAddress, "Contract Node Address")}
              >
                <Copy className="h-4 w-4" />
              </Button>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <Input value={project.contractHash} readOnly className="bg-muted flex-1 font-mono text-sm" />
              <Button
                variant="outline"
                size="icon"
                onClick={() => copyToClipboard(project.contractHash, "Contract Hash")}
              >
                <Copy className="h-4 w-4" />
              </Button>
            </div>
          ...
          {project.description && (
            <div className="space-y-2">
              ...
              <Input value={project.description} readOnly className="bg-muted" />
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card className="hover:shadow-elevated transition-all duration-300">
        <CardHeader>
          ...
          <CardDescription>Use these credentials to connect your application</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">Safe to use in client-side code</p>
          </div>
          <div className="space-y-2">
            ...
            <p className="text-xs text-destructive">Never share or commit this key to version control</p>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-primary/20 hover:shadow-elevated transition-all duration-300">
        <CardHeader>
          ...
          <CardDescription>Follow these steps to integrate Triggr into your application</CardDescription>
        </CardHeader>
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="flex-1">
                ...
                <div className="bg-console-bg border border-console-border rounded p-3">
                  <code className="text-sm font-mono text-foreground">npm install @triggr/sdk</code>
                </div>
              </div>
            </div>
